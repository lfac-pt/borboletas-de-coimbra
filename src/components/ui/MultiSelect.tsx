...
const MultiSelect = ({
  ...
  const getDisplayValue = () => {
    ...
    if (selected.length === 1)
      return valueLabelMap
        ? valueLabelMap[selected[0]] || selected[0]
        : selected[0];
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        <span className="truncate block max-w-[90%]">{getDisplayValue()}</span>
        <svg
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-100 rounded-lg shadow-xl max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
          <div className="p-1 space-y-0.5">
            ...
            {options.map((option) => {
              ...
              const displayLabel = valueLabelMap
                ? valueLabelMap[option] || option
                : option;
              ...
              return (
                ...
                >
                  <div
                    className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? "bg-[#2d6a4f] border-[#2d6a4f]" : "border-gray-300 bg-white"}`}
                  >
                    {isSelected && (
                      <svg
